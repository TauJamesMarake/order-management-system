...
//
// PHASE 1 CHANGES:
//   verifyToken — now resolves user + business in one query.
//     Stamps business_id onto req.user.
//     Rejects suspended businesses with HTTP 403.
//
//   verifyPlatformToken — NEW.
//     Authenticates platform operators via platform_admins table.
//     Completely separate code path from verifyToken.
//     Stamps req.platformAdmin (never req.user).
//
// ARCHITECTURE — two round-trips per authenticated request:
//   1. Supabase Auth: anonClient.auth.getUser(token) — JWT verification
//   2. Single SQL:    users JOIN businesses!inner     — profile + tenant check
//
//   The user + business lookup is ONE query (inner join), not two.
//   This keeps the overhead to the minimum necessary.
// ─────────────────────────────────────────────────────────────
...
// Used for JWT verification only (auth.getUser).
...
// ── toOneRecord ───────────────────────────────────────────────
// Supabase JS types all nested relation joins as T | T[] at
// compile time — it cannot infer cardinality from the FK direction.
// At runtime, a many-to-one join always returns a single object.
// This utility satisfies TypeScript without unsafe double-casting.
...
export async function verifyToken(
  ...
  try {
    ...
    //
    // We select only what we need — not SELECT * — to keep the
    // payload minimal on every authenticated request.
    ...
    //
    // toOneRecord() resolves the Supabase JS compile-time ambiguity:
    // the type is inferred as T | T[] but the runtime value for a
    // many-to-one FK join is always a single object.
