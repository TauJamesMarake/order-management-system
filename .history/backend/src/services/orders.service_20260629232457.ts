...
export async function createOrder(
  dto: CreateOrderDTO,
  createdById: string,
  businessId: string        // ★ NEW — always from req.user.business_id in controller
): Promise<Order> {
  const order_number = await generateOrderNumber(businessId)  // ★ pass businessId

  const { data, error } = await supabase
    .from('orders')
    .insert({
      order_number,
      business_id:    businessId,      // ★ NEW — tenant discriminator
