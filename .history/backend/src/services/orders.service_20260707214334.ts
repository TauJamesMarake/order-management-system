...
//
// PHASE 2 CHANGES:
//   Every public function now receives businessId as an explicit
//   parameter and applies .eq('business_id', businessId) on every
//   Supabase query. The parameter is always sourced from the
//   verified JWT via the controller — never from the client.
//
//   createOrder   — signature already updated in Phase 0/1 guidance;
//                   delivered complete here.
//   getOrders     — businessId added; scoped on v_orders_with_creator
//   getOrderById  — businessId added; cross-tenant → 404
//   updateOrder   — businessId propagated through getOrderById
//   cancelOrder   — businessId propagated through getOrderById
//   getOrderSummary — redesigned:
//     OLD: SELECT status FROM orders (all rows, JS reduce)  O(n) rows
//     NEW: parallel COUNT queries + server-side SUM         O(1) rows
//
// CROSS-TENANT PROTECTION:
//   A request for a resource that exists in another tenant returns
//   "not found" (→ 404), never "forbidden" (→ 403).
//   This prevents confirmation that the resource exists elsewhere.
// ─────────────────────────────────────────────────────────────
...
export async function createOrder(
  dto:          CreateOrderDTO,
  createdById:  string,
  businessId:   string,          // ★ PHASE 2 — always from req.user.business_id
): Promise<Order> {
