    ...
    return (
        ...
        >
            ...
            <div style={{ padding: '20px 16px', borderTop: `1px solid ${T.charcoal}`, display: 'flex', flexDirection: 'column', gap: 4 }}>
                <button
    onClick={() => setIsSettingsOpen(true)} // Added click listener
    style={{
        display: 'flex',
        alignItems: 'center',
        gap: 14,
        padding: '12px 18px',
        borderRadius: 16,
        backgroundColor: isSettingsOpen ? 'rgba(255,255,255,0.08)' : 'transparent', // Dynamic highlight active state
        border: 'none',
        width: '100%',
        textAlign: 'left',
        cursor: 'pointer',
        color: isSettingsOpen ? T.white : T.inkGhost, // Sync text color
        transition: 'all 0.2s',
    }}
    onMouseEnter={(e) => (e.currentTarget.style.color = T.white)}
    onMouseLeave={(e) => {
        if (!isSettingsOpen) e.currentTarget.style.color = T.inkGhost // Maintain color when open
    }}
>
    <SettingsIcon color={isSettingsOpen ? T.teal : 'currentColor'} /> {/* Highlight icon when open */}
