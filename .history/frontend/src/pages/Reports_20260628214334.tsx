...
export function Reports() {
          ...
          <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
            <div style={{ 
              ...
              <p style={{ margin: 0, color: T.inkSecondary, fontSize: 14, lineHeight: 1.5 }}>
                Your active tenant configuration profile scope ({user.role}) possesses insufficient authority attributes to aggregate live operational analytics parameters.[cite: 1]
              </p>
            </div>
          </main>
        </div>
  ...
  const handleExport = async (format: 'excel' | 'pdf') => {
    try {
      ...
      window.open(`${import.meta.env.VITE_API_URL || ''}/api/reports/export/${format}`, '_blank')[cite: 1]
    } catch (error) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 12, color: T.inkGhost }}>Exports are compiled directly in-memory and securely bounded by tenant context fields.[cite: 1]</p>
              </div>
              ...
              <>
                ...
                <div style={{ backgroundColor: T.white, borderRadius: 20, padding: 24, boxShadow: '0 4px 20px rgba(14,31,31,0.61)', border: `1px solid ${T.mutedCream}60` }}>
                  <h3 style={{ margin: '0 0 20px', fontSize: 15, fontWeight: 700, color: T.inkPrimary, borderBottom: `1px solid ${T.panelBg}`, paddingBottom: 12 }}>
                    Commodity Yield Distribution Ledger[cite: 1]
                  </h3>
