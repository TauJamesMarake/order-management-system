...
//
// WHY BUFFERS NOT FILES:
//   Railway/Render filesystems are ephemeral — writing to disk
//   breaks on redeploy. In-memory buffers work anywhere,
//   need no cleanup, and handle concurrent requests safely.
//
// TWO OUTPUTS:
...
// Status badge colours for Excel — matches SRS status lifecycle
...
export async function buildExcelReport(
  ...
  // Column definitions — widths tuned to content
  sheet.columns = [
  ...
  titleRow.getCell(1).value = 'NTSOAKI DISTRIBUTIONS — ORDER REPORT'
...
export async function buildPdfReport(
  ...
  // jsPDF is a CommonJS module — dynamic import avoids TS issues
  ...
  doc.setFontSize(13)
  doc.text('NTSOAKI DISTRIBUTIONS', margin, 8)
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Page ${i} of ${pageCount}   |   Ntsoaki Distributions — Confidential`,
