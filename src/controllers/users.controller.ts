 ...
 * Users Controller
 *
 * ── Change: iCreateUser / iUpdateUser removed ─────────────────────────────
 * types/iusers.ts defined iCreateUser and iUpdateUser manually.  Those
 * interfaces duplicated the shape already inferred by the Zod schemas here
 * and were a second source of truth that could silently drift.
 *
 * The types are now derived directly from the schemas via z.infer<> so there
 * is only one authoritative definition.
 */
